...
SD_Error SD_PowerON(void)
{
  ...
  if (errorstatus == SD_OK)	//响应了cmd55，是sd卡，可能为1.x,可能为2.0
  {
    ...
    while ((!validvoltage) && (count < 0x1ff/*SD_MAX_VOLT_TRIAL*/))
    {	 
...
#define SDIO_FORCE_TIMEOUT  2000 
 /*
 * 函数名：CmdResp1Error
 * 描述  ：对响应类型为R1的命令进行检查
 * 输入  ：无
...
{		   
  uint32_t timeout = 0;
	/*不是这些状态就等待	*/
  while ((timeout < SDIO_FORCE_TIMEOUT) && (!(SDIO->STA & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CMDREND | SDIO_FLAG_CTIMEOUT))))
  {
		if(SDisNotExist)
			break;
        timeout++;
...
static SD_Error CmdResp3Error(void)
{
  ...
  while ((timeout < SDIO_FORCE_TIMEOUT) && (!(status & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CMDREND | SDIO_FLAG_CTIMEOUT))))
  {
    status = SDIO->STA;
    timeout++;
  }
...
static SD_Error CmdResp2Error(void)
{
  ...
  while ((timeout < SDIO_FORCE_TIMEOUT) && (!(status & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CTIMEOUT | SDIO_FLAG_CMDREND))))
  {
    status = SDIO->STA;
    timeout++;
  }
